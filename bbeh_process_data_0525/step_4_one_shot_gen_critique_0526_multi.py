    ...
    for process_file in glob.glob(file_pattern):
        if os.path.exists(process_file):
            ...
            for q_id, result in process_results.items():
                # 只更新未处理或无效的结果
                if q_id not in merged_results or not merged_results[q_id].get("cft_answer_valid", False):
        ...
        if os.path.exists(process_file):
            ...
            for q_id, result in process_results.items():
                if q_id not in existing_results or (
                        result.get("cft_answer_valid", False) and
                        not existing_results[q_id].get("cft_answer_valid", False)
                ):
...
def get_questions_to_process(input_data, existing_results, start_idx=0, end_idx=20000):
    ...
    for q_data in input_data:
        ...
        if True:
            # 跳过已经成功处理的问题
            if q_data["critique_id"] in existing_results and existing_results[q_data["critique_id"]].get("answer_valid", False):
...
if __name__ == "__main__":
    ...
    INPUT_FILE = "3_add_critique_data/add_critique_data_0526.json"
    OUTPUT_FILE = f"3_add_critique_data/add_critique_data_{teacher_model_name}_0526.json"  # 结果输出文件路径
    NUM_PROCESSES = 2  # 进程数
