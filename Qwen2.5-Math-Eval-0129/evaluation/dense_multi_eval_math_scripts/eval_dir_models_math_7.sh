...
cd /gpfs/public/research/xy/yubowang/CriticCoTT/Qwen2.5-Math-Eval/evaluation/sh
...
for checkpoint_dir in ${models_dir}/checkpoint-*; do
    if [ -d "$checkpoint_dir" ]; then  # 确保是目录
        ...
        # 检查checkpoint_num是否[100, 130)
        if [ "$checkpoint_num" -ge 48 ] && [ "$checkpoint_num" -lt 56 ]; then
            # 设置输出目录
