...
class JSONDataset(Dataset):
    """
    Dataset class for loading JSON format data files
    Supports both single JSON file and JSONL (JSON Lines) format
    """
    ...
    def __init__(self,
        ...
        self.original_json_files = copy.deepcopy(json_files)  # use for resume
        self.cache_dir = os.path.expanduser(cache_dir)
        self.tokenizer = tokenizer
    ...
    def _download(self, use_origin_json=False):
        ...
        for i, json_file in enumerate(json_files):
            self.json_files[i] = copy_local_path_from_hdfs(src=json_file, cache_dir=self.cache_dir)
    ...
    def _read_files_and_tokenize(self):
        ...
        # filter out too long prompts
        if self.filter_prompts and len(self.dataframe) > 0:
            tokenizer = self.tokenizer
            prompt_key = self.prompt_key
            
            def check_prompt_length(doc):
                try:
                    if isinstance(doc[prompt_key], list) and len(doc[prompt_key]) > 0:
                        content = doc[prompt_key][0].get('content', '')
                    elif isinstance(doc[prompt_key], dict):
                        content = doc[prompt_key].get('content', '')
                    else:
                        content = str(doc[prompt_key])
                    return len(content) <= self.max_prompt_length
                except:
                    return False
    ...
    def __getitem__(self, item):
        ...
        if isinstance(chat, list) and len(chat) > 0:
            prompt_with_chat_template = chat[0].get('content', '')
