...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        from verl.utils.dataset.rl_dataset import RLHFDataset, JSONDataset
        
        # Determine data format and create appropriate dataset
        train_files = self.config.data.train_files
        val_files = self.config.data.val_files
        
        # Check if files are JSON format
        def is_json_format(files):
            if isinstance(files, str):
                return files.endswith('.json') or files.endswith('.jsonl')
        ...
        if use_json:
            # Use JSONDataset for JSON files
            ...
            # Use RLHFDataset for parquet files (original behavior)
        ...
        # Create validation dataset with same format
        if use_json:
        ...
        print(f'Data format: {"JSON" if use_json else "Parquet"}')
