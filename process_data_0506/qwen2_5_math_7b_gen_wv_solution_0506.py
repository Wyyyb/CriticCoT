...
def preprocess_long_text(output_data):
    ...
    for k, v in output_data.items():
        if "qwen2.5-math-7b_answer" in v and len(v["qwen2.5-math-7b_answer"]) > 50000:
            print("too long text", len(v["qwen2.5-math-7b_answer"]))
            output_data[k]["qwen2.5-math-7b_answer_valid"] = False
            long_text_count += 1
...
def get_process_data(output_data):
    process_data = []
    sta_count = {"qwen2.5-math-7b_answer_valid": 0, "qwen2.5-math-7b_answer_correct": 0,
                 "qwen2.5-math-7b_answer_invalid": 0, "qwen2.5-math-7b_answer_incorrect": 0,
                 "too long": 0}
    for k, v in output_data.items():
        if "qwen2.5-math-7b_answer_valid" in v and v["qwen2.5-math-7b_answer_valid"] is True:
            sta_count["qwen2.5-math-7b_answer_valid"] += 1
            if v.get("qwen2.5-math-7b_answer_correctness") is True:
                sta_count["qwen2.5-math-7b_answer_correct"] += 1
            else:
                sta_count["qwen2.5-math-7b_answer_incorrect"] += 1
            continue
        sta_count["qwen2.5-math-7b_answer_invalid"] += 1
    ...
    while len(process_data) > 500:
        ...
        for batch_idx in range(total_batches):
            ...
            for i, output in enumerate(batch_output):
                ...
                output_data[question]["qwen2.5-math-7b_answer"] = output
                extracted_answer = extract_boxed_answer(output)
                if extracted_answer is None:
                    output_data[question]["qwen2.5-math-7b_answer_valid"] = False
                    batch_sta["invalid_num"] += 1
                else:
                    output_data[question]["qwen2.5-math-7b_answer_valid"] = True
                    batch_sta["valid_num"] += 1
                    if extracted_answer == output_data[question]["gt_answer"]:
                        output_data[question]["qwen2.5-math-7b_answer_correctness"] = True
                        batch_sta["right_num"] += 1
                    else:
                        output_data[question]["qwen2.5-math-7b_answer_correctness"] = False
