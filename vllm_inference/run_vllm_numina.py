...
def single_file_inference(file_path, model_path):
    output_path = file_path.replace(".jsonl", "_on_policy.jsonl")
...
def main(model_path, input_dir):
    for each in os.listdir(os.path.join(input_dir, "math_train")):
        if not each.endswith(".jsonl"):
            continue
        single_file_critique(os.path.join(input_dir, "math_train", each), model_path)
    return
