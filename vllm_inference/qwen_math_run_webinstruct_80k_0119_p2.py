...
def main():
    input_file = "/gpfs/public/research/xy/yubowang/CriticCoT/LLaMA-Factory/data/webinstruct_cft_80k_0119.json"
    ...
    with open(input_file, "r") as fi:
        webinstruct_data = json.load(fi)
    input_data = []
    prompts = []
    idx = 0
    if os.path.exists(output_file):
        with open(output_file, "r") as fi:
            exists_data = json.load(fi)
    exists_questions = []
    for each in exists_data:
        if each.get("qwen_2.5_math_answer", "") != "":
            exists_questions.append(each.get("question"))

    for each in webinstruct_data:
        question = each["input"].split("Solution:\n")[0].replace("Question:\n", "")
        # question = each["question"].replace("Question:\n", "")
        if question in exists_questions:
            continue
    ...
    batch_size = 4000
    # batch_size = 40
    batch_num = len(prompts) // batch_size + 1
    outputs = []
    ...
    for each in exists_data:
        output_data.append({"question": each["question"], "qwen_2.5_math_answer": each["qwen_2.5_math_answer"]})
    print("final output data length: ", len(output_data))
