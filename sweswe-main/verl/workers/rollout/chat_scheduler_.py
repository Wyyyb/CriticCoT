...
class R2ECompletionCallback(CompletionCallback):
    ...
    async def __call__(self, messages: List[Dict[str, str]], completions: ChatCompletion, info: Dict[str, Any]):
        ...
        f = open(f"output_example/output_{batch_index}.txt", "a")
    ...
    def postprocess(self, batch: DataProto, batch_conversations: List[List[Dict[str, str]]], rewards: List[float], n: int) -> DataProto:
        # NOTE: consistent with batch version of generate_sequences in vllm_rollout_spmd.py
        # prompts: left pad
        # responses: right pad
        # input_ids: prompt + response
        ...
        num_turns = np.array([len(conversation) for conversation in batch_conversations], dtype=np.int32)
        return DataProto(batch=batch, non_tensor_batch={"__rewards__": np.array(rewards), "__num_turns__": num_turns})
...
class R2EChatCompletionScheduler:
    def __init__(
        ...
        server_addresses: List[str],
        max_cache_size: int = 10000,
        max_concurrent_tasks: int = 64,  # 新增参数：最大并发任务数
    ):
        """
        Args:
            config: DictConfig.
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        for batch_index, conversation in enumerate(batch.non_tensor_batch["raw_prompt"].repeat(n, axis=0)):
            ...
            with open(f"output_example/output_{batch_index}.txt", "w") as f:
                pass
            tasks.append(
                asyncio.create_task(
                    self._submit_chat_completions_semaphore(
        ...
        await asyncio.gather(*tasks)
        output_batch = self.completion_callback.postprocess(batch, batch_conversations, rewards, n=n)
        output_batch.meta_info["timing"] = {"generate_sequences": time.time() - t_start}
